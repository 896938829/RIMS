...
- `audit` — append-only audit log (`audit_logs` table via `types.BaseModel`, no `AuditableModel` — audit IS the audit); captures `traceId` / `userId` / `username` / `roleCode` / `warehouseId` / `action` / `resource` / `resourceId` / `docNo` / `ipAddress` / `userAgent` / `result` + `errorCode`/`errorMsg` and a JSONB `details` column holding `{before, after}` snapshots. `AuditService.Log(ctx, Entry)` uses `db.FromCtx` so writes inside `db.RunInTx` commit/rollback atomically with the outer business tx (requirements §5.4/§8.2); outside a tx it is best-effort. `Entry.Actor` (built via `audit.ActorFromContext(c)`) is how handlers thread IP/UA/traceId into services whose plain `context.Context` can't reach gin helpers. Read surface is admin-only: `GET /api/v1/audit/logs` (filter by user/warehouse/resource/action/docNo/time/result, max 366d window) + `GET /api/v1/audit/logs/:id`; admin gate lives in the handler per project convention, route group is NOT warehouse-scoped (audit spans user/role mgmt). No write endpoint. Pilot consumers: `user.Handler.Login` (best-effort, records both success and failure with AppError code/msg) and `document.Service.Complete` (in-tx, with before/after doc status snapshot). Every consumer declares its own local `AuditLogger` interface that `*audit.AuditService` structurally satisfies. Retrofitting the rest of the write sites is a README-tracked follow-up.
